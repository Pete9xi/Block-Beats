...
createArchiveButton.addEventListener('click', async () => {
  try {
    ...
    await fetch(`http://portal.oshosting.co.uk:3000/update_version/${userId}`, {
      method: 'POST',
    ...
    await fetch(`http://portal.oshosting.co.uk:3000/update_json/${userId}`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: updatedJson,
    });
    ...
    await fetch(`http://portal.oshosting.co.uk:3000/upload_audio/${userId}`, {
      method: 'POST',
      body: formData,
    });
    ...
    await fetch(`http://portal.oshosting.co.uk:3000/cleanup/${userId}`);
